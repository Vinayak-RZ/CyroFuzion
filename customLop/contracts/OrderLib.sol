...
bytes32 private constant _LIMIT_ORDER_TYPEHASH =
    keccak256(
        "Order("
        "uint256 salt,"
        "address maker,"
        ...
        "uint256 makerTraits,"
        "bytes32 crossChainRecipient,"
        "bytes32 suiAsset)"
    );

// Updated size constants for bytes32 fields
// Each bytes32 field is exactly 1 slot (32 bytes)
uint256 internal constant _ORDER_STRUCT_SIZE = 0x140; // 10 * 32 bytes (9 original + 2 for bytes32 fields)
uint256 internal constant _DATA_HASH_SIZE = 0x140; // 10 * 32 bytes (no additional hashing overhead for bytes32)
